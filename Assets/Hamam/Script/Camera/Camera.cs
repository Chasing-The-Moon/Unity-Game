...
public class Camera : MonoBehaviour
{
    ...
    [HideInInspector] [SerializeField] float OrthographicSize; // to take the OrthographicSize from the Virtual Camera
    public int CameraNumber;
    [HideInInspector] public CinemachineFramingTransposer VarCinemachineFramingTransposer; // m_GroupFramingSize
    ...
    [HideInInspector] public bool IsCharacter2Sleeping; // character1.GetComponent<Rigidbody2D>().IsSleeping() 
    ...
    [HideInInspector] public float ZoomOutSpeed = 5f; // the Speed of the Zoom out
    [HideInInspector] public float ZoomOutValueTarget = 10f; // The new OrthographicSize value when we want to do the Zoom out in Zoom Out Function
    
    [HideInInspector]
    [Header("Zoom In Values")]
    public bool ZoomIn; // the Condition to do the Zoom In Function
    [HideInInspector] public float ZoomInSpeed = 3f;  // the Speed of the Zoom out
    [HideInInspector] public float ZoomInValueTarget = 1.2f;  // The new OrthographicSize value when we want to do the Zoom In in Zoom In Function

    [HideInInspector]
    [Header("Origin Zoom Values")]
    [SerializeField] bool OriginZoom;  // the Condition to do the Zoom out Function
    [HideInInspector] [SerializeField] float OriginZoomSpeed = 1f;   // the Speed of the Zoom out , Currently we are not using it
    [HideInInspector] [SerializeField] float OriginZoomValue; // will be equal to the standard zoom == OrthographicSize == 5f or if we will change it , it will be used in the Standard Zoom Function
    ...
    [SerializeField] List<GameObject> ObjectsListToFollow;

    // make new object to follow is list and depnds on that we change the the things , camera oject is the same
    [Header("Event Periority")]
    [HideInInspector] public bool EventChangeCamera;
    [HideInInspector] [SerializeField] int CameraCurrnetPriority;  // not usable variable only in the start function
    [HideInInspector] [SerializeField] int CameraNewPriority;  // a variable to take the Event Camera Poriority , so from it we can swap and blending between the main camera and the event camera by changing the current periority
    [HideInInspector] [SerializeField] int CameraOriginPriority; // a variable to save the camera origin priority before we change it or doing swap between the periorities of the cameras (save main Camera Priority)

    [Header("New Camera Object [Event Camera]")]
    public GameObject NewCameraOject; // to take the seccond camera which is the Event camera , Camera Event

    [Header("Character Idle")]
    [HideInInspector] public bool IdleZoomOut; // the Condition to do the Idle Zoom out Function
    [HideInInspector] [SerializeField] float IdleZoomOutSpeed = 5f; // the Speed of the Idle Zoom out 5f , 2f
    [HideInInspector] public float IdleZoomOutValueTarget = 0.001f; // The new OrthographicSize value when we want to do the Zoom out in Zoom Out Function  0.001
    [HideInInspector] public float IdleZoomOutOriginValue = 0.42f; // the origin value we can tak it from the group framin size from the start by assigning it to that variable
    [HideInInspector] public bool restofconditions;
    ...
    [Header("Event")]
    [HideInInspector] public int CurrentEventNumber; // inisde change camera we put if AND THE NUMBER OF THE EVENT WE WANT
    ...
    [HideInInspector] public float StartEventAfterTimer;
    [HideInInspector] public float EndEventAfterTimer;
    ...
    public void BlendTimer()
    {
        if (CameraBrain != null)
        {

            CameraBrain.m_DefaultBlend.m_Time = TransitionTimer;
        }
    }
    ...
    public void IdleFunction()
    {
        if (CameraNumber == 1)
        {
            if(EventChangeCamera ==  true || Pausemenue.gameObject.activeSelf == true ) // if we swapping a camera dont do it
            {
                IdleZoomOut = false;
            }
            if (IdleZoomOut) 
            {
                VarCinemachineFramingTransposer.m_GroupFramingSize = Mathf.Lerp(VarCinemachineFramingTransposer.m_GroupFramingSize, 0.001f, (Time.deltaTime / 2f));
            }
            if (!SleepingCondition()) // the condition if the character axis is not moving for long time is true , is mean the state is Awaking which is moving , so if one of the characters are moving then don't do the idle zoom
            {
                ...
                {
                    VarCinemachineFramingTransposer.m_GroupFramingSize = IdleZoomOutOriginValue;
                }
            }
        }
    }
    ...
    {
        if(CurrentEventNumber == 1) // 1 is for doing the zoom to an object
        {
            if (EventChangeCamera)  // the condition to change between the cameras Periorities
            {
                NewObjectToFollow1 = TheNewObjectToFollow(NumberOfObjectToFollow);
                ...
                yield return new WaitForSeconds(StartEventAfterTimer); // is better to be the same of the blending time to start together , the event will start in X secconds
                ...
                yield return new WaitForSeconds(EndEventAfterTimer); // the seccond timer is to make the event finish after X secconds
            ...
            else
            {
                ...
                FinishZoomOutEvents.Invoke(); // Bryan line
                StartCoroutine("CameraCharacterIdle"); // when it is falls we restart make the start coroutin working again
            }
        }
        if(CurrentEventNumber == 2) // 2 is to make the camera go to the object and then back to the character
        {
            if (EventChangeCamera)
            {
                ...
                yield return new WaitForSeconds(StartEventAfterTimer); // 0.5f
                BlendTimer();
                CameraNewPriority = NewCameraOject.GetComponent<CinemachineVirtualCamera>().m_Priority - 5; // change camera periority to the new one
                virtualCamera.m_Priority = CameraNewPriority; // setting the values
                Debug.Log("Entering Event Number ==2 , Changing the follow and priority");
                yield return new WaitForSeconds(EndEventAfterTimer); // 5f
                EventChangeCamera = false;
                StartCoroutine("MyTesting");
            }
            else
        ...
        if (CurrentEventNumber == 3) // 1 is for doing the zoom to an object
        {
            if (EventChangeCamera)  // the condition to change between the cameras Periorities
            {
                NewObjectToFollow1 = TheNewObjectToFollow(NumberOfObjectToFollow);
                ...
                virtualCamera.m_Priority = CameraNewPriority; // setting the values
                Debug.Log("first timer start 4 secconds to start zoom in");
                yield return new WaitForSeconds(StartEventAfterTimer); // 4f
                Debug.Log("first timer finish after 4 secconds now zoom in");
                NewCameraOject.GetComponent<Camera>().ZoomOut = true; // to make the new camera do the zoom out event

                Debug.Log("secconds timer start after 6 secconds  is false");
                yield return new WaitForSeconds(EndEventAfterTimer); //6f
                Debug.Log("secconds timer finish after 6 secconds now zoom in  condition is false");
                EventChangeCamera = false;
                StartCoroutine("MyTesting");
            }
            else
            {
                CameraNewPriority = CameraOriginPriority;
                virtualCamera.m_Priority = CameraNewPriority;
                NewCameraOject.GetComponent<Camera>().ZoomOut = false;
                yield return new WaitForSeconds(3f);
                NewCameraOject.GetComponent<CinemachineVirtualCamera>().m_Lens.OrthographicSize = OrthographicSize;
                FinishZoomOutEvents.Invoke(); // Bryan line
                StartCoroutine("CameraCharacterIdle"); // when it is falls we restart make the start coroutin working again
    ...
    public GameObject TheNewObjectToFollow( int i)
    {
        NewObjectToFollow1 = ObjectsListToFollow[i];
        return NewObjectToFollow1;
    }
    ...
    public void ZoomInTest2() // the Main Function to do the Zoom In
    {
        if (ZoomIn)
        {
            if (virtualCamera.m_Lens.OrthographicSize < ZoomInValueTarget + 0.01f)
            {
